$(function () {
    $("#btnGrabarContratoPagos").click(function () {
...
function grabarMantenimientoSalario() {
    ...
    var entidad = {
        ...
        ID_ENTIDAD: $("#DdlEntidadModal").val(),
    ...
    $.ajax({
        type: 'POST',
        url: baseUrl + "Carga/Contrato/EditarPago",
        data: JSON.stringify(entidad),
        contentType: 'application/json; charset=utf-8',
        success: function (result) {
            if (result) {
                $("#modalCargaPago").modal("hide");
            ...
            if ($("#txhParentModal").val() === "CONTRATO") {
                window.parent.ContratoPagos.ListarPagos();

                jQuery("#modalCargaPagoModal").html('');
                $('#modalCargaPagoModal').modal('hide');
            } else {
                window.parent.Contratos.ListarPagos();

                jQuery("#modalCargaPago").html('');
                $('#modalCargaPago').modal('hide');
            }
...
$("button[name='btnSalirContratoPagosModal']").click(function () {
    if ($("#txhParentModal").val() === "CONTRATO") {
        jQuery("#modalCargaPagoModal").html('');
        $('#modalCargaPagoModal').modal('hide');
    } else {
